...
function AccountStatusBadge({ user }: { user: UserAdminResponse }) {
  if (user.deletedAt)
    return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-red-100 text-red-800 border-red-200">Supprimé</span>;
  if (!user.enabled)
    return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-orange-100 text-orange-800 border-orange-200">Désactivé</span>;
  if (!user.emailVerified)
    return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-amber-100 text-amber-800 border-amber-200">Non vérifié</span>;
  return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-emerald-100 text-emerald-800 border-emerald-200">Actif</span>;
}
...
export default function AdminUsersPage() {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error((d as {error?: string}).error ?? "Impossible de supprimer l'utilisateur.");
      }
  ...
  const RoleBadges = ({ user }: { user: UserAdminResponse }) => {
    ...
    return (
      <div className="flex flex-wrap items-center gap-1.5">
        {user.roles.map((role) => {
          ...
          return (
            <span key={role} className={cn("inline-flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-full text-xs font-medium border", cfg.className)}>
              {cfg.label}
              <button
          ...
          addingRoleFor === user.id ? (
            <div className="flex items-center gap-1">
              <select
                ...
                defaultValue=""
                onChange={(e) => { if (e.target.value) handleRoleAction(user.id, e.target.value as Role, "ADD"); }}
              >
              ...
              <button onClick={() => setAddingRoleFor(null)} className="text-[#6a6a6a] hover:text-[#222222] dark:hover:text-[#f0f0f0]">
                <X className="w-3 h-3" />
              </button>
            </div>
          ) : (
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="w-full sm:w-auto h-9 rounded-[8px] border border-[#dddddd] px-3 text-sm text-[#222222] bg-white dark:bg-[#2a2a2a] dark:border-[#3a3a3a] dark:text-[#f0f0f0] focus:outline-none"
        >
      ...
      {isLoading ? (
        <>
          ...
          <div className="md:hidden space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-32 rounded-[14px] bg-[#f2f2f2] dark:bg-[#2a2a2a] animate-pulse" />
            ))}
          </div>
          {/* Desktop skeleton */}
          <div className="hidden md:flex items-center justify-center py-16 text-[#6a6a6a]">Chargement…</div>
        </>
      ) : users.length === 0 ? (
            ...
            {users.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className={cn(
                    "rounded-[14px] border border-[#dddddd] dark:border-[#3a3a3a] bg-white dark:bg-[#1a1a1a] p-4 space-y-3",
                    user.deletedAt && "opacity-60"
                  )}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-semibold text-[#222222] dark:text-[#f0f0f0]">{user.firstName} {user.lastName}</p>
                      <p className="text-xs text-[#929292] mt-0.5">{user.email}</p>
                    </div>
                    <AccountStatusBadge user={user} />
                  </div>
                  <div>
                    <p className="text-xs font-medium text-[#6a6a6a] mb-1.5">Rôles</p>
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    >
                      ...
                      {isConfirm ? "Confirmer la suppression" : "Supprimer"}
                    </Button>
                    {isConfirm && (
                      <button className="text-xs text-[#6a6a6a] hover:text-[#222222]" onClick={() => setConfirmId(null)}>
                        Annuler
                      </button>
                    )}
                  </div>
      ...
      {pageInfo && pageInfo.totalPages > 1 && (
        <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-[#6a6a6a]">
          <span>Page {pageInfo.page + 1} sur {pageInfo.totalPages} ({pageInfo.totalElements} utilisateurs)</span>
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" disabled={!pageInfo.hasPrevious} onClick={() => setPage((p) => p - 1)}>
              Précédent
            </Button>
            <Button size="sm" variant="outline" disabled={!pageInfo.hasNext} onClick={() => setPage((p) => p + 1)}>
              Suivant
            </Button>
          </div>
        </div>
